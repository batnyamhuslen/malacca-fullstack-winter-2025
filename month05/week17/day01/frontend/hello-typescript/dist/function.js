...
// ── Optional parameter ─────────────────────────────────────────────────────
function buildSearchUrl(title, rating) {
...
// ── Default parameter ─────────────────────────────────────────────────────
function getFilmsUrl(page = 1, size = 20) {
...
const filterByRating = (films, rating) => films.filter(f => f.rating === rating);
// Кино хэдэн доллар байгааг буцаах
function formatPrice(price) {
    return `$${price.toFixed(2)}`;
}
// Film array-г дамжуулж rental_rate-г нийлбэрлэх
function totalRentalRate(films) {
